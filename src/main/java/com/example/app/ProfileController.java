...
public class ProfileController {
  ...
  public void initialize() {
    ...
    usernameLabel.setText(SessionManager
        .getUsername() != null ? SessionManager.getUsername() : unknown);
  ...
  public void handleChangePassword() {
    ...
    if (current.isEmpty()) {
      showMessage(LocaleManager
          .getString("profile.error.currentPasswordRequired"), true);
      return;
    }
    if (newPass.isEmpty()) {
      showMessage(LocaleManager
          .getString("profile.error.newPasswordRequired"), true);
      return;
    }
    if (confirm.isEmpty()) {
      showMessage(LocaleManager
          .getString("profile.error.confirmPasswordRequired"), true);
      return;
    }
    if (!newPass.equals(confirm)) {
      showMessage(LocaleManager
          .getString("profile.error.passwordMismatch"), true);
      return;
    }
    if (newPass.equals(current)) {
      showMessage(LocaleManager
          .getString("profile.error.passwordDifferent"), true);
      return;
    }
    if (newPass.length() < 6) {
      showMessage(LocaleManager
          .getString("profile.error.passwordMinLength"), true);
      return;
    }
  ...
  private void changePassword(String currentPassword, String newPassword) {
    try {
      ...
      if (response.statusCode() == 200) {
        ...
        showMessage(LocaleManager
            .getString("profile.error.passwordChangeFailed", response.statusCode()), true);
      }
